# Original validation 이미지에서 Bounding Box 부분(json 파일에 있음)을 crop하여 새로운 이미지 생성
'''
    - input
        - 원본 validation 이미지 파일 (Dataset/validation/validation/image/)
        - json 파일 (Dataset/validation/validation/annos/)

    - output
        - cropped 이미지 파일 (Dataset/validation/validation/cropped_image/)
'''
import os
import json
...
# Json, 이미지 폴더 지정
json_folder = "Dataset/validation/validation/annos/"
image_folder = "Dataset/validation/validation/image/"

# 생성되는 Cropped 이미지 저장 폴더 지정
cropped_image_folder = "Dataset/validation/validation/cropped_image/"
